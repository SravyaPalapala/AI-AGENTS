...
st.title("🤖🚀 RAGnetic AI (Flash Thinking and Agno)")
...
def create_vector_store(collection, texts):
    try:
        ...
        with st.spinner('📤 Loading documents to Chroma...'):
            for text in texts:
...
def retrieve_documents(prompt, client, COLLECTION_NAME):
    try:
        # Simple search without unsupported parameters
        ...
        for result in results:
            if hasattr(result, 'page_content'):
                ...
                # Handle different document formats silently
        ...
        return processed_results, len(processed_results) > 0
...
if st.session_state.google_api_key:
    ...
    st.sidebar.header("📁 Post Data here")
        ...
        if file_name not in st.session_state.processed_documents:
            with st.spinner('Loading PDF...'):
                ...
                if texts and collection_chroma:
                    ...
                    st.success(f"✅ Checked in PDF: {file_name}")
        ...
        if web_url not in st.session_state.processed_documents:
            with st.spinner('Processing URL...'):
                ...
                if texts and collection_chroma:
                    ...
                    st.success(f"✅ Checked in URL: {web_url}")

    if st.session_state.processed_documents:
        st.sidebar.header("📚 Loaded Sources")
